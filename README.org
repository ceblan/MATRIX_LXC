  ...
  - name: Set up LXC container for a EMACS-30
    ...
    tasks:
      ...
      - name: create git-carlos
        command: lxc-attach -n {{ DEST }} -- mkdir -p /git-carlos

      - name: chown git-carlos
        command: lxc-attach -n {{ DEST }} -- chown {{ emacs_username }}:devpl /git-carlos
      ...
      - name: Add mount entry to /bar/baz
        lineinfile:
          path: /var/lib/lxc/{{ DEST }}/config
          line: "lxc.mount.entry = /home/sice/git-sice git-carlos none bind 0 0"
      ...
      - name: Pause for 3 seconds (using command)
        command: sleep 3
